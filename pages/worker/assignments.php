  ...
  try {
    $stmt = $db->prepare(
      'SELECT a.id AS assignment_id, a.issue_id, a.notes, a.assigned_at, a.completed_at,
              ...
              i.priority_score, i.created_at, i.upvote_count,
    ...
    $availableStmt = $db->query(
      "SELECT i.id, i.title, i.category, i.address, i.priority_score,
        ...
        GROUP BY i.id, i.title, i.category, i.address, i.priority_score, i.created_at
        ORDER BY i.priority_score DESC, i.created_at DESC
        LIMIT 30"
    );
    $availableIssues = $availableStmt->fetchAll(PDO::FETCH_ASSOC);
...
<body class="app-body">
  <div class="app-shell">
    <aside id="workerSidebar" class="app-sidebar" data-app-sidebar aria-label="Worker navigation">
      ...
      <nav class="nav flex-column gap-1"><a class="nav-link active" href="assignments.php"><i class="fas fa-clipboard-list fa-fw"></i><span>Assignments</span><span class="nav-count"><?= $assignmentStats['active'] ?></span></a><a class="nav-link" href="../citizen/public-feed.php"><i class="fas fa-layer-group fa-fw"></i><span>Community feed</span></a><a class="nav-link" href="../heatmap/"><i class="fas fa-map-location-dot fa-fw"></i><span>City pulse</span></a><a class="nav-link" href="../account/profile.php"><i class="fas fa-user fa-fw"></i><span>Profile</span></a></nav>
      <div class="app-sidebar-spacer"></div>
      <div class="app-sidebar-user"><span class="app-avatar"><?= $e(strtoupper(substr((string) ($viewer['name'] ?? 'W'), 0, 1))) ?></span><div><strong><?= $e($viewer['name'] ?? 'Worker') ?></strong><small><?= $e($viewer['city'] ?? 'Field worker') ?></small></div></div><a class="nav-link text-danger mt-2" href="../logout/"><i class="fas fa-arrow-right-from-bracket fa-fw"></i><span>Sign out</span></a>
    </aside>
    <div class="app-sidebar-backdrop" data-sidebar-backdrop="workerSidebar"></div>

    <main class="app-content"><header class="app-topbar"><div class="d-flex align-items-center gap-3"><button class="app-menu-toggle" data-sidebar-toggle="workerSidebar" aria-controls="workerSidebar" aria-expanded="true" aria-label="Toggle navigation"><i class="fas fa-bars"></i></button><div><div class="eyebrow">Field operations</div><h1>My assignments</h1></div></div><a class="btn btn-sm btn-outline-secondary" href="../citizen/public-feed.php"><i class="fas fa-eye me-1"></i>Public view</a></header>
  ...
  <script>
    ...
    document.querySelectorAll('[data-status-save]').forEach((button) => button.addEventListener('click', async () => {
      ...
      try {
        const response = await fetch('../../api/issues/status.php', {method: 'POST', headers: {'Content-Type': 'application/json', 'X-CSRF-Token': workerCsrfToken}, body: JSON.stringify({issue_id: issueId, status: select.value})});
    ...
    requestForm?.addEventListener('submit', async (event) => {
      ...
      button.disabled = true;
      try {
        const response = await fetch('../../api/work/request.php', {method: 'POST', headers: {'Content-Type': 'application/json', 'X-CSRF-Token': workerCsrfToken}, body: JSON.stringify({issue_id: issueId, message})});
        const result = await response.json();
    ...
    document.querySelectorAll('[data-complete-id]').forEach((button) => button.addEventListener('click', async () => {
      ...
      button.disabled = true;
      try {
        const response = await fetch('../../api/issues/status.php', {method: 'POST', headers: {'Content-Type': 'application/json', 'X-CSRF-Token': workerCsrfToken}, body: JSON.stringify({issue_id: issueId, status: 'resolved', note: 'Marked completed by worker.'})});
        const result = await response.json();
