...
$viewer = sessionUser() ?? [];
$assignmentData = fetchAssignmentsPageData($db instanceof PDO ? $db : null, (int) ($viewer['id'] ?? 0));
extract($assignmentData);
...
<body class="app-body">
  <div class="app-shell">
    <aside id="workerSidebar" class="app-sidebar" data-app-sidebar aria-label="Worker navigation">
      ...
      <nav class="nav flex-column gap-1"><a class="nav-link active" href="<?= $e($urlForAssignments) ?>"><i class="fas fa-clipboard-list fa-fw"></i><span>Assignments</span><span class="nav-count"><?= $assignmentStats['active'] ?></span></a><a class="nav-link" href="<?= $e($urlForPublicFeed) ?>"><i class="fas fa-layer-group fa-fw"></i><span>Community feed</span></a><a class="nav-link" href="<?= $e($urlForHeatmap) ?>"><i class="fas fa-map-location-dot fa-fw"></i><span>City pulse</span></a><a class="nav-link" href="<?= $e($urlForProfile) ?>"><i class="fas fa-user fa-fw"></i><span>Profile</span></a><a class="nav-link" href="<?= $e($urlForDashboard) ?>"><i class="fas fa-chart-pie fa-fw"></i><span>Dashboard</span></a></nav>
      <div class="app-sidebar-spacer"></div>
      <div class="app-sidebar-user"><span class="app-avatar"><?= $e(strtoupper(substr((string) ($viewer['name'] ?? 'W'), 0, 1))) ?></span><div><strong><?= $e($viewer['name'] ?? 'Worker') ?></strong><small><?= $e($viewer['city'] ?? 'Field worker') ?></small></div></div><a class="nav-link text-danger mt-2" href="<?= $e($urlForLogout) ?>"><i class="fas fa-arrow-right-from-bracket fa-fw"></i><span>Sign out</span></a>
    </aside>
    <div class="app-sidebar-backdrop" data-sidebar-backdrop="workerSidebar"></div>

    <main class="app-content"><header class="app-topbar"><div class="d-flex align-items-center gap-3"><button class="app-menu-toggle" data-sidebar-toggle="workerSidebar" aria-controls="workerSidebar" aria-expanded="true" aria-label="Toggle navigation"><i class="fas fa-bars"></i></button><div><div class="eyebrow">Field operations</div><h1>My assignments</h1></div></div><a class="btn btn-sm btn-outline-secondary" href="<?= $e($urlForPublicFeed) ?>"><i class="fas fa-eye me-1"></i>Public view</a></header>
  ...
  <script>
    ...
    const statusEndpoint = <?= json_encode($urlForApi . 'issues/status.php') ?>;
    const workRequestEndpoint = <?= json_encode($urlForApi . 'work/request.php') ?>;
