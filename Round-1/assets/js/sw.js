...
self.addEventListener("fetch", (event) => {
  if (
    event.request.url.includes("/api/issues/submit") &&
  ...
  for (const item of queued) {
    try {
      ...
      for (const entry of item.entries)
        formData.append(entry.name, entry.value, entry.filename || undefined);
      ...
      if (!response.ok)
        throw new Error(`Submission retry failed with ${response.status}`);
      ...
      resolve({
        getAll: () =>
          new Promise((ok, fail) => {
            ...
            const get = tx.objectStore(STORE_NAME).getAll();
        ...
        add: (item) =>
          new Promise((ok, fail) => {
            ...
            const add = tx.objectStore(STORE_NAME).add(item);
            add.onsuccess = () => ok(add.result);
            add.onerror = () => fail(add.error);
          }),
        delete: (id) =>
          new Promise((ok, fail) => {
            const tx = database.transaction(STORE_NAME, "readwrite");
            const del = tx.objectStore(STORE_NAME).delete(id);
            del.onsuccess = () => ok();
...
self.addEventListener("online", () =>
  self.registration.sync?.register("sync-issues"),
);
